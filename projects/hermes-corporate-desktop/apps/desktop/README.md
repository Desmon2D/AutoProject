...
**DIT Agent Desktop is a downstream corporate prototype based on
[Hermes Agent](https://github.com/NousResearch/hermes-agent) by Nous Research.**
It keeps the Hermes backend and desktop architecture while adding an isolated
development profile, DIT branding, and separately deployed corporate MCP
integrations. There is currently no official prebuilt DIT Agent installer.
...
<tr><td><b>Voice</b></td><td>Talk to DIT Agent and hear it back.</td></tr>
<tr><td><b>Settings & onboarding</b></td><td>Manage providers, models, tools, and credentials from a real UI. First-run setup gets you to your first message in seconds.</td></tr>
<tr><td><b>Update channel</b></td><td>Must be pointed at a controlled DIT release source before distribution.</td></tr>
</table>
...
Install workspace dependencies from the repository root, then start the isolated
Windows profile:
...
This stores Hermes runtime data and Electron state under
`%LOCALAPPDATA%\hermes-corporate-dev`, separate from a normal Hermes install.
The local backend command remains `hermes serve`; internal protocol and config
names keep their upstream `HERMES_*` identifiers for compatibility.

---

## Updating

Do not publish the upstream update controls as a DIT release channel. Before
distributing installers, point update checks at the fork's controlled release
source and verify that an update cannot replace the DIT build with upstream
Hermes.
...
The upstream non-isolated development command remains available:
...
first launch it can install the DIT Agent runtime into `HERMES_HOME`
...
- **DIT Agent** runs as a headless `hermes serve` process and exposes the
