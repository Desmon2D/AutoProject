...
export const ja = defineLocale({
  ...
  boot: {
    ...
    steps: {
      ...
      startingHermesDesktop: 'DIT Agent を起動中…'
    },
    errors: {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。DIT Agent を再起動してください。',
  ...
  settings: {
    ...
    gateway: {
      ...
      intro:
        'DIT Agent はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Hermes バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。ゲートウェイ接続はマシン単位の設定で、プロファイルは接続したゲートウェイから検出されます。',
      ...
      restartingMessage: 'DIT Agent は保存された設定を使用して再接続します。',
      ...
      sshErrPlatform:
        'サポートされていないリモートプラットフォームです。DIT Agent の SSH モードは Linux、macOS、Windows のリモートホストに対応しています。',
  ...
  install: {
    ...
    setupChoiceTitle: 'DIT Agent をセットアップ',
    setupChoiceDesc:
    ...
    localStartUnavailable:
      'ローカルインストールを開始できません。DIT Agent を再起動して、もう一度お試しください。',
    remoteSetupTitle: '既存の Hermes に接続',
    remoteSetupDesc:
      'ゲートウェイ URL を入力してください。DIT Agent がトークンとブラウザーサインインのどちらが必要かを検出します。',
  ...
  onboarding: {
    headerTitle: 'DIT Agent のセットアップをしましょう',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには DIT Agent を再起動してください。',
    restartToSaveImages: '画像を保存するには DIT Agent を再起動してください',
