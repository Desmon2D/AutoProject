...
export const zh: Translations = {
  ...
  notifications: {
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 DIT Agent。',
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        'DIT Agent 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 Hermes 后端时，可以使用远程网关。网关连接属于本机级设置；profile 是从所连接的网关中发现的。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: 'DIT Agent 将使用已保存设置重新连接（界面保持打开）。',
      ...
      sshErrPlatform: '不支持的远程平台。DIT Agent 的 SSH 模式支持 Linux、macOS 和 Windows 远程主机。',
  ...
  install: {
    ...
    localStartUnavailable: '无法启动本地安装。请重启 DIT Agent 后重试。',
    remoteSetupTitle: '连接到现有 Hermes',
    remoteSetupDesc: '输入网关 URL。DIT Agent 会检测需要令牌还是浏览器登录。',
