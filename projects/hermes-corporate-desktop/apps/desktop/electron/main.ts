...
const APP_NAME = process.env.HERMES_DESKTOP_APP_NAME || 'DIT Agent'
...
// assets/dit-agent.ico in development (icon.ico in packaged resources) and only falls
...
// electron-builder derives from the build `appId` (com.dit.agent) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.dit.agent')
}
...
function resolveHermesBackend(backendArgs) {
  ...
  return {
    ...
    label: `${APP_NAME} not installed yet; bootstrap required`,
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function startHermes() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('DIT Agent is already running in another window.')
  }
